...
async def get_user_by_email(db: AsyncSession, email: str) -> user_schemas.User:
    async with db as session:
        result = await session.exec(select(database_models.User).where(database_models.User.email == email))
        return result.first()


async def get_users(db: AsyncSession, skip: int = 0, limit: int = 100) -> Sequence[user_schemas.User]:
    async with db as session:
        results = await session.exec(select(database_models.User).offset(skip).limit(limit))
        return results.all()
...
async def create_user(db: AsyncSession, user: user_schemas.UserCreate) -> user_schemas.User:
    ...
    async with db as session:
        ...
        session.add(database_models.UserSettings(user_id=db_user.id))
        await session.commit()
        return db_user


async def update_user(db: AsyncSession, db_user: database_models.User, user: user_schemas.UserUpdate
                      ) -> user_schemas.User:
